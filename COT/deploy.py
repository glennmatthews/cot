...
def deploy_esxi(UI,
                ...
                ovftool_args=[],
                **kwargs):
    # ensure user entered a locator
    if LOCATOR is None:
        raise InvalidInputError("No locator was specified.")
    ...
    if password is None:
        password = UI.get_password(username, server)
    ...
    with VMContextManager(PACKAGE, None) as vm:
        ...
        if vm.get_property_keys() and not re.search("/host/", LOCATOR):
            if get_ovftool_version() < StrictVersion("4.0.0"):
                UI.confirm_or_die("When deploying an OVF directly to a vSphere "
        ...
        if profile_list and configuration is None:
            ...
            while configuration is None:
                ...
                user_input = UI.get_input("Choose a Configuration:", "0")
...
def create_subparser(parent):
    import argparse
    # This becomes problematic once we have multiple 'deploy' subcommands
    # but for now it's OK to do everything in here...

    # Create a generic parser with arguments to be shared by all
    generic = argparse.ArgumentParser(add_help=False)

    generic.add_argument('-u', '--username', help="Server login username")

    generic.add_argument('-p', '--password', help="Server login password")

    generic.add_argument('-c', '--configuration', help=
...
"multiple mappings.")
    ...
    generic.set_defaults(func=deploy)

    # Create 'cot deploy' parser
    gp = parent.add_parser(
        'deploy',
    ...
    gp.add_argument('PACKAGE', help="""OVF descriptor or OVA file""")

    hypervisor_subparsers = gp.add_subparsers(
        dest='HYPERVISOR', metavar='hypervisors supported:')

    gp.set_defaults(func=deploy)

    # Create 'cot deploy ... esxi' parser
    p = hypervisor_subparsers.add_parser(
        'esxi', parents=[generic],
        usage=("""
  ...
  {0} deploy foo.ova esxi 192.0.2.100 -u admin -p password \\
    ...
    # ovftool uses '-ds' as shorthand for '--datastore', so let's allow it too.
    p.add_argument("-d", "-ds", "--datastore", help=
"ESXi datastore to use for the new VM")

    p.add_argument("-o", "--ovftool-args", help=
"Quoted string describing additional CLI parameters to pass through "
"""to "ovftool". Examples: -o="--foo", --ovftool-args="--foo --bar" """)

    p.add_argument("LOCATOR", help=
...
'"192.0.2.101/mydatacenter/host/192.0.2.100" (deploy via vCenter server)')
    ...
    p.set_defaults(func=deploy_esxi)
